        ...
        if (!$records) {
            ...
            if (!$DB->insert_record('plagiarism_plagkh_request', $request)) {
                \plagiarism_plagkh_logs::add(
                    "ошибка добавления новой записи в очередь для cmid: " .
                        $data["courseModuleId"] . ", конец очереди: $endpoint",
    ...
    public static function update_plagkh_eula_version($version) {
        ...
        if ($configeula) {
            ...
            if (!$DB->update_record('plagiarism_plagkh_config', $configeula)) {
                \plagiarism_plagkh_logs::add(
                    "Ошибка обновления пользовательского соглашения до версии: $version",
    ...
    public static function upsert_eula_by_user_id($userid) {
        ...
        if (!$user) {
            if (!$DB->insert_record('plagiarism_plagkh_users', array('userid' => $userid))) {
                \plagiarism_plagkh_logs::add(
                    "ошибка вставки новой записи в базу данных для: " .
                        "plagiarism_plagkh_users, Не создано новой пользовательской записи для пользователя $userid",
                    "INSERT_RECORD_FAILED"
                );
            }
        }
        ...
        if (
            ...
            \plagiarism_plagkh_logs::add(
                "ошибка вставки новой записи в базу данных для:" .
                    "plagiarism_plagkh_eula, Не создано новой пользовательской записи о пользовательском соглашении для пользователя $userid",
                "INSERT_RECORD_FAILED"
            );
        }
    }
