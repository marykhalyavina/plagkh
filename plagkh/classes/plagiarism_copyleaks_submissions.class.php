...
class plagiarism_plagkh_submissions {
    ...
    public static function create($cm, $userid, $identifier, $submissiontype) {
        ...
        if (!$fileid = $DB->insert_record('plagiarism_plagkh_files', $file)) {
            \plagiarism_plagkh_logs::add(
                "не удалось вставить запись базы данных для cmid: " . $cm->id . ", userid: " . $userid,
    ...
    public static function save(
        ...
        if ($fileid != 0) {
            if (!$DB->update_record('plagiarism_plagkh_files', $file)) {
                \plagiarism_plagkh_logs::add(
                    "не удалось обновить запись базы данных для cmid: "
                        . $cm->id . ", userid: " . $userid,
            ...
            if (!$DB->insert_record('plagiarism_plagkh_files', $file)) {
                \plagiarism_plagkh_logs::add(
                    "не удалось вставить запись базы данных для cmid: "
                        . $cm->id . ", userid: " . $userid,
                    "INSERT_RECORD_FAILED"
                );
            }
    ...
    public static function mark_error($fileid, $errormsg = null) {
        ...
        if (!$DB->update_record('plagiarism_plagkh_files', $file)) {
            \plagiarism_plagkh_logs::add(
                "не удалось обновить запись базы данных для fileid: " . $fileid,
                "UPDATE_RECORD_FAILED"
            );
        } else {
